...
AGSHeroCharacter::AGSHeroCharacter(const class FObjectInitializer& ObjectInitializer) : Super(ObjectInitializer)
{
	...
	InputHandler= CreateDefaultSubobject<UInputHandlerComponent>(FName("InputHandler"));
}
...
void AGSHeroCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	PlayerInputComponent->BindAction("Up", IE_Pressed, this, &AGSHeroCharacter::UpKey<true>);
	PlayerInputComponent->BindAction("Up", IE_Released, this, &AGSHeroCharacter::UpKey<false>);

	PlayerInputComponent->BindAction("Down", IE_Pressed, this, &AGSHeroCharacter::DownKey<true>);
	PlayerInputComponent->BindAction("Down", IE_Released, this, &AGSHeroCharacter::DownKey<false>);

	PlayerInputComponent->BindAction("Left", IE_Pressed, this, &AGSHeroCharacter::LeftKey<true>);
	PlayerInputComponent->BindAction("Left", IE_Released, this, &AGSHeroCharacter::LeftKey<false>);

	PlayerInputComponent->BindAction("Right", IE_Pressed, this, &AGSHeroCharacter::RightKey<true>);
	PlayerInputComponent->BindAction("Right", IE_Released, this, &AGSHeroCharacter::RightKey<false>);

	PlayerInputComponent->BindAction("Right", IE_Released, this, &AGSHeroCharacter::RightKey<false>);
	PlayerInputComponent->BindAction("Right", IE_Released, this, &AGSHeroCharacter::RightKey<false>);

	PlayerInputComponent->BindAction("Ctrl", IE_Pressed, this, &AGSHeroCharacter::CtrlKey<true>);
	PlayerInputComponent->BindAction("Ctrl", IE_Released, this, &AGSHeroCharacter::CtrlKey<false>);

	PlayerInputComponent->BindAction("Alt", IE_Pressed, this, &AGSHeroCharacter::AltKey<true>);
	PlayerInputComponent->BindAction("Alt", IE_Released, this, &AGSHeroCharacter::AltKey<false>);

	PlayerInputComponent->BindAction("PrimaryFire", IE_Pressed, this, &AGSHeroCharacter::OnPrimaryActionPressed<true>);
	PlayerInputComponent->BindAction("PrimaryFire", IE_Released, this, &AGSHeroCharacter::OnPrimaryActionPressed<false>);
...
void AGSHeroCharacter::PossessedBy(AController* NewController)
{
	...
	if (PS)
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("AGSHeroCharacter::PossessedBy %s, max health:%f"), *GetName(),GetMaxHealth());
		AttributeSetBase->RecalculateMaxHealthAfterRestriction();
		SetHealth(GetMaxHealth());
		SetMana(GetMaxMana());
		SetMeleeAttackRate(GetBaseMeleeAttackRate());

		UE_LOG(LogTemp, Warning, TEXT("AGSHeroCharacter::Set bow draw rate:%f"), GetBaseBowDrawRate());
		SetBowDrawRate(GetBaseBowDrawRate());
		SetMagicAttackRate(GetBaseMagicAttackRate());
		AttributeSetBase->RecalculateMaxStaminaAfterRestriction();
		AttributeSetBase->RecalculateMaxManaAfterRestriction();
		SetStamina(GetMaxStamina());
		if (AbilitySystemComponent->GetTagCount(DeadTag) > 0)
		{
			// Set Health/Mana/Stamina to their max. This is only necessary for *Respawn*.
			UE_LOG(LogTemp, Warning, TEXT("AGSHeroCharacter::PossessedBy, max health:%f"), GetMaxHealth());
			//SetHealth(GetMaxHealth());
...
void AGSHeroCharacter::FinishDying()
{
	...
	UE_LOG(LogTemp, Warning, TEXT("AGSHeroCharacter::FinishDying"));
	if (!HasAuthority())
	{
		return;
	}
...
void AGSHeroCharacter::EquipWeapon(AGSWeapon* NewWeapon)
{
	...
	{
		EWeaponEquipSlot EquipSlot=NewWeapon->GetWeaponEquipSlot();
		if (WeaponEquipMap.Contains(EquipSlot))
		{
			SetCurrentWeapon(NewWeapon, WeaponEquipMap[EquipSlot]);
		}
		else
		{

			SetCurrentWeapon(NewWeapon, nullptr);
		}
	}
}
...
{
	return CurrentWeapon->WeaponAttachPoint;
}
...
void AGSHeroCharacter::BeginPlay()
{
	...
	FGenericTeamId::SetAttitudeSolver([](FGenericTeamId  ID1, FGenericTeamId  ID2) {
		...
		UE_LOG(LogTemp, Warning, TEXT("AGSHeroCharacter, solve, ID1:%i, ID2:%i"), ID1.GetId(), ID2.GetId());
		if (ID1.GetId() == ID2.GetId())
		{
			return ETeamAttitude::Friendly;
		}

		UE_LOG(LogTemp, Warning, TEXT("AGSHeroCharacter, solve,hostile"));
		return ETeamAttitude::Hostile; }
	); 
	AttributeSetBase->RecalculateMaxHealthAfterRestriction();
	SetHealth(GetMaxHealth());

	SetGenericTeamId(FGenericTeamId(TeamIDNum));
}

void AGSHeroCharacter::ImportStatFromTable()
{
	TArray< FPlayerTableStat*> PlayerTable;
	MyStatTable->GetAllRows<FPlayerTableStat>("Name", PlayerTable);
	for (auto FoundStat : PlayerTable)
	{
		if (FoundStat->ActorType == GetClass())
		{
			MyPlayerStat = FoundStat->PlayerStat;
		}
    }
}
...
template <bool bPressed>
void AGSHeroCharacter::UpKey()
{
	InputHandler->Registerkey(EAdditionalHelperInput::UpKey,bPressed);
}

template <bool bPressed>
void AGSHeroCharacter::DownKey()
{

	InputHandler->Registerkey(EAdditionalHelperInput::DownKey, bPressed);
}

template <bool bPressed>
void AGSHeroCharacter::RightKey()
{

	InputHandler->Registerkey(EAdditionalHelperInput::RightKey, bPressed);
}

template <bool bPressed>
void AGSHeroCharacter::LeftKey()
{

	InputHandler->Registerkey(EAdditionalHelperInput::LeftKey, bPressed);
}

template <bool bPressed>
void AGSHeroCharacter::OnPrimaryActionPressed()
{

	InputHandler->RegisterAction(EGSAbilityInputID::PrimaryFire, bPressed);
}

template <bool bPressed>
void AGSHeroCharacter::OnSecondaryActionPressed()
{

	InputHandler->RegisterAction(EGSAbilityInputID::SecondaryFire, bPressed);
}

template <bool bPressed>
void AGSHeroCharacter::CtrlKey()
{

	InputHandler->Registerkey(EAdditionalHelperInput::CtrlKey, bPressed);
}

template<bool bPressed>
void AGSHeroCharacter::AltKey()
{
	UE_LOG(LogTemp, Warning, TEXT("void AGSHeroCharacter::AltKey"));
	InputHandler->Registerkey(EAdditionalHelperInput::AltKey, bPressed);
}
...
void AGSHeroCharacter::MoveForward(float Value)
{
	...
	if (IsAlive())
	{
		if (GSCharMovement->RequestToStartSprinting)
		{
			if (Value != 0)
			{
				AddMovementInput(UKismetMathLibrary::GetForwardVector(FRotator(0, GetControlRotation().Yaw, 0)), 1);
			}
...
void AGSHeroCharacter::SetCurrentWeapon(AGSWeapon* NewWeapon, AGSWeapon* LastWeapon)
{
	...
	if (NewWeapon)
	{
		...
		if (NewWeapon->GetWeaponEquipSlot() == EWeaponEquipSlot::EWES_Mainhand)
		{
		...
		if (WeaponEquipMap.Contains(EquipSlot))
		{
			WeaponEquipMap[EquipSlot] = NewWeapon;
		}
		else
		{

			WeaponEquipMap.Add(EquipSlot, NewWeapon);
		}
		NewWeapon->SetOwningCharacter(this);
		NewWeapon->Equip();
		...
		if (PC && PC->IsLocalController())
		{
			PC->SetEquippedWeaponPrimaryIconFromSprite(NewWeapon->PrimaryIcon);
			PC->SetEquippedWeaponStatusText(NewWeapon->StatusText);
			PC->SetPrimaryClipAmmo(NewWeapon->GetPrimaryClipAmmo());
			PC->SetPrimaryReserveAmmo(GetPrimaryReserveAmmo());
			PC->SetHUDReticle(NewWeapon->GetPrimaryHUDReticleClass());
		}
		...
		UAnimMontage* Equip1PMontage = NewWeapon->GetEquip1PMontage();
		if (Equip1PMontage && GetFirstPersonMesh())
		{
			GetFirstPersonMesh()->GetAnimInstance()->Montage_Play(Equip1PMontage);
		}

		UAnimMontage* Equip3PMontage = NewWeapon->GetEquip3PMontage();
...
void AGSHeroCharacter::UnEquipWeapon(AGSWeapon* WeaponToUnEquip)
{
	...
	if (WeaponToUnEquip)
	{
		...
		EWeaponEquipSlot EquipSlot = WeaponToUnEquip->GetWeaponEquipSlot();
		if (WeaponEquipMap.Contains(EquipSlot))
		{
			WeaponEquipMap.Remove(EquipSlot);
		}
...
{
	UE_LOG(LogTemp, Warning, TEXT("AGSHeroCharacter::UnEquipCurrentWeapon"));
	if (AbilitySystemComponent)
	{
		AbilitySystemComponent->RemoveLooseGameplayTag(CurrentWeaponTag);
		CurrentWeaponTag = NoWeaponTag;
...
void AGSHeroCharacter::ClientSyncCurrentWeapon_Implementation(AGSWeapon* InWeapon)
{
	if (InWeapon)
	{
		UE_LOG(LogTemp, Warning, TEXT("AGSHeroCharacter::SetCurrentWeapon:%s"), *InWeapon->GetName());
	}
	else
	{
		UE_LOG(LogTemp, Warning, TEXT("AGSHeroCharacter::SetCurrentWeapon, none"));
	}
...
void AGSHeroCharacter::SetGenericTeamId(const FGenericTeamId& NewTeamID)
{
	TeamID = NewTeamID;
}

UBlendSpace* AGSHeroCharacter::GetIdleWalkRunTouse()
{
	if (AnimDataAsset&&CurrentWeapon)
	{
		if (AnimDataAsset->MyIdleWalkRunAnimMap.Contains(CurrentWeapon->GetWeaponType()))
		{
			return AnimDataAsset->MyIdleWalkRunAnimMap[CurrentWeapon->GetWeaponType()].BSIdleWalkRun;
		}
	}

	if (!CurrentWeapon)
	{
		if (AnimDataAsset->MyIdleWalkRunAnimMap.Contains(EWeaponType::EWT_UnArmed))
		{
			return AnimDataAsset->MyIdleWalkRunAnimMap[EWeaponType::EWT_UnArmed].BSIdleWalkRun;
		}
	}
	return nullptr;
}

UAimOffsetBlendSpace1D* AGSHeroCharacter::GetAimOffsetToUse()
{
	if (AnimDataAsset&&CurrentWeapon)
	{
		if (AnimDataAsset->MyIdleWalkRunAnimMap.Contains(CurrentWeapon->GetWeaponType()))
		{
			return AnimDataAsset->MyIdleWalkRunAnimMap[CurrentWeapon->GetWeaponType()].AimOffSet;
		}
	}
	if (!CurrentWeapon)
	{
		if (AnimDataAsset->MyIdleWalkRunAnimMap.Contains(EWeaponType::EWT_UnArmed))
		{
			return AnimDataAsset->MyIdleWalkRunAnimMap[EWeaponType::EWT_UnArmed].AimOffSet;
		}
	}
	return nullptr;
}

UAnimSequence* AGSHeroCharacter::GetBlockingAnim()
{
	for (auto MapInfo : WeaponEquipMap)
	{
		if (MapInfo.Value->GetWeaponType() == EWeaponType::EWT_Shield)
		{
			return MapInfo.Value->BlockingAnim;
		}
	}
	if (CurrentWeapon)
	{
