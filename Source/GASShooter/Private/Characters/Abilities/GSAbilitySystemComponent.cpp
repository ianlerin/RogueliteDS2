...
void UGSAbilitySystemComponent::AbilityLocalInputPressed(int32 InputID)
{
	...
	TArray<EAdditionalHelperInput> PressedInput;
	EMovementMode MovementMode = EMovementMode::MOVE_None;
	// ---------------------------------------------------------
	APlayerState* State = Cast< APlayerState>(GetOwner());
	if (State)
	{
		AGSHeroCharacter* HeroChar =Cast<AGSHeroCharacter>(State->GetPawn());
		if (HeroChar)
		{
			PressedInput = HeroChar->GetInputHandlerComp()->GetPressedInput();
		}
		UCharacterMovementComponent* CharMovementComp = Cast<UCharacterMovementComponent>(HeroChar->GetMovementComponent());
		if (CharMovementComp)
		{
			MovementMode = CharMovementComp->MovementMode;
		}
	}
		...
		if (Spec.InputID == InputID)
		{
			if (Spec.Ability)
			{
				...
				else
				{
					...
					bool bFullFillMovement = true;
					if (GA->bRequireSpecificMovement)
					{
					
						if (MovementMode != GA->RequiredMovementMode)
						{
							continue;
						}
					}
					if (GA && GA->bActivateOnInput&&bFullFillMovement)
					{
					if (GA->AdditionalInput.Num() > 0)
					{
						for (auto InputRequired : GA->AdditionalInput)
						{
							if (PressedInput.Find(InputRequired)<0)
							{
								bHasPerfectMatch = false;
							}
					...
					if (bHasPerfectMatch)
					{
						bool bActivate=TryActivateAbility(Spec.Handle);
						UE_LOG(LogTemp, Warning, TEXT("UGSAbilitySystemComponent::AbilityLocalInputPressed, found perfect:%s,bActivate:%i"),*Spec.Ability->GetName(), bActivate);
...
void UGSAbilitySystemComponent::Damage(UGSGameplayAbility* DamageAbility, FGameplayAbilityTargetDataHandle TargetData, FComboChainData ChainData)
{
	TArray<AActor*>Collection=UAbilitySystemBlueprintLibrary::GetActorsFromTargetData(TargetData, 0);
	for (int i = 0;i < Collection.Num(); i++)
	{
		FRotator Rot = UKismetMathLibrary::FindLookAtRotation(Collection[i]->GetActorLocation(), GetAvatarActor()->GetActorLocation());
		auto Comp=UAbilitySystemBlueprintLibrary::GetAbilitySystemComponent(Collection[i]);
		if (Comp)
		{
			bool bIsBlocking = Comp->HasMatchingGameplayTag(FGameplayTag::RequestGameplayTag("Status.Block"));
			bool bIsParrying= Comp->HasMatchingGameplayTag(FGameplayTag::RequestGameplayTag("Status.Parry"));
			if (bIsParrying&&ChainData.bParriable&&FMath::Abs(Rot.Yaw)<45)
			{
				// apply parry effect
			}
...
void UGSAbilitySystemComponent::SetAbilityLevel(TSubclassOf<UGameplayAbility> AbilityClass, int32 Level)
{
	FGameplayAbilitySpecHandle SpecHandle = FindAbilitySpecHandleForClass(AbilityClass);
	UGSGameplayAbility* GSAbility=UGSBlueprintFunctionLibrary::GetPrimaryAbilityInstanceFromHandle(this,SpecHandle);
	FGameplayAbilitySpec* Spec = FindAbilitySpecFromHandle(SpecHandle);
	if (Spec)
	{
		int32 OldLevel = Spec->Level;
		Spec->Level = Level;
		if (GSAbility)
		{
			UE_LOG(LogTemp, Warning, TEXT("UGSAbilitySystemComponent::SetAbilityLevel:%s, level:%i"), *AbilityClass.Get()->GetName(), Level);
			GSAbility->OnLevelChange(Spec->Level,OldLevel);
		}
	}
}