...
UGSGameplayAbility::UGSGameplayAbility()
{
	...
	ActivationBlockedTags.AddTag(FGameplayTag::RequestGameplayTag("Status.Stagger"));
	ActivationBlockedTags.AddTag(FGameplayTag::RequestGameplayTag("Status.LiftUp"));
	ActivationBlockedTags.AddTag(FGameplayTag::RequestGameplayTag("Status.Roll"));
	ActivationBlockedTags.AddTag(FGameplayTag::RequestGameplayTag("Status.Parried"));
	ActivationOwnedTags.AddTag(FGameplayTag::RequestGameplayTag("Ability.BlocksInteraction"));
	//ActivationOwnedTags.AddTag(FGameplayTag::RequestGameplayTag("Event.CancelChanelling"));
	CancelAbilitiesWithTag.AddTag(FGameplayTag::RequestGameplayTag("Status.Chanelling"));
...
void UGSGameplayAbility::OnAvatarSet(const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilitySpec& Spec)
{
	...
	OnConfirmDelegate.AddUFunction(this, "OnActivateAbilityDelegate");
}
...
void UGSGameplayAbility::EndAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, bool bReplicateEndAbility, bool bWasCancelled) 
{
	...
	case(EElementalType::Fire):
		K2_RemoveGameplayCue(FGameplayTag::RequestGameplayTag("GameplayCue.Element.Casting.Fire"));
		break;
	case(EElementalType::Ice):
		K2_RemoveGameplayCue(FGameplayTag::RequestGameplayTag("GameplayCue.Element.Casting.Ice"));
		break;
	}
...
{
	UE_LOG(LogTemp, Warning, TEXT(" UGSGameplayAbility::ApplyCost:%s,ResourceRestriction:%i"), *GetName(), ResourceRestriction.Num());
	...
	for (auto Restriction : ResourceRestriction)
	{
		if (Hero)
		{
			UE_LOG(LogTemp, Warning, TEXT(" UGSGameplayAbility::OnResourceRestrictionAdd:%s"), *GetName());
			Hero->OnResourceRestrictionAdd(this, Restriction.Key, Restriction.Value);
		}
	}
}
...
void UGSGameplayAbility::OnLevelChange(int32 NewLevel,int32 OldLevel)
{
	OnAbilityLevelChangeDelegate.Broadcast(NewLevel, OldLevel);
}

void UGSGameplayAbility::OnActivateAbilityDelegate()
{
	UE_LOG(LogTemp, Warning, TEXT("UGSGameplayAbility::OnActivateAbilityDelegate,fire"));
	switch (ElementType)
	{
	case(EElementalType::Fire):
		K2_AddGameplayCue(FGameplayTag::RequestGameplayTag("GameplayCue.Element.Casting.Fire"),GetGrantedByEffectContext());
		break;
	case(EElementalType::Ice):
		K2_AddGameplayCue(FGameplayTag::RequestGameplayTag("GameplayCue.Element.Casting.Ice"), GetGrantedByEffectContext());
		break;
	}
}

void UGSGameplayAbility::ActivateAbility(const FGameplayAbilitySpecHandle Handle, const FGameplayAbilityActorInfo* ActorInfo, const FGameplayAbilityActivationInfo ActivationInfo, const FGameplayEventData* TriggerEventData)
{
	Super::ActivateAbility(Handle, ActorInfo, ActivationInfo, TriggerEventData);
	OnActivateAbilityDelegate();
}